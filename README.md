...
> ### ⚠️ 本分支：Android 16 兼容版
>
> 自 **2025-10 Android 安全补丁**（AOSP bug 441823943 / **CVE-2025-48617**）起，
> `ICarrierConfigLoader.overrideConfig` 会直接拒绝 shell 调用，上游 Nrfr 在 Android 16 上
> 必然报 **`overrideConfig cannot be invoked by shell`**。
>
> 本分支改用框架自带的 **CarrierService 扩展点**（`setCarrierTestOverride` +
> `setCarrierServicePackageOverride`）实现同样的效果，**依然免 Root**，并自动兼容未打补丁的旧系统。
>
> 详见 **[docs/ANDROID16.md](docs/ANDROID16.md)**。
